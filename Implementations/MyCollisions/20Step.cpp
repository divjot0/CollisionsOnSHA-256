...
void fillMessageRandomlyAtStep(AVarSHA2 aVar, AVarSHA2 aVar2, EVarSHA2 eVar, EVarSHA2 eVar2, vector<vector<vector<char> > > &differentials, vector<unsigned int> &w, vector<unsigned int> &w2, int step, int &state, bool checkAll) {
	...
	if (checkAll) {
		...
		else if (step==6 || step==7 || step==8 || step==9 || step==10 || step==11 || step==12) {
			state = state+1;
			unsigned int eNum = 0;
		...
		else if (step==6 || step==9 || step==10 || step==11 || step==12) {
...
bool conditionsStatisfied(AVarSHA2 aVar, EVarSHA2 eVar, vector<unsigned int> w, AVarSHA2 aVar2, EVarSHA2 eVar2, vector<unsigned int> w2, vector<vector<vector<char> > > &differentials, int step) {
	...
	if (step==6) {
		cout << "cing out wrong bits of a[6]: ";
...
void fillMessageAtStep(AVarSHA2 &aVar, EVarSHA2 &eVar, vector<unsigned int> &w, AVarSHA2 &aVar2, EVarSHA2 &eVar2, vector<unsigned int> &w2, vector<vector<vector<char> > > &differentials, Conditions &conditions, int step) {
	...
	if (step==6) {
		eight++;
		cout << "Step 6 : " << eight << endl;
		// printMessages(w, w2);
	}
	if (step==7 && prevPrinted!=7) {
		cout << "Step 7 : " << endl;
		prevPrinted = 7;
		printMessages(w, w2);
		// checkAll = false;
	...
	while(!done) {
		if (checkAll) {
			...
			if (step==4) {
				if (trials>thres) {
					if (countNext>thresBottom) {
						...
						thres = 10*thres;
						cout << "extending 4 to " << thres << endl;
					}
					else {
						return;
					}
			...
			if (step==7 || step==8 || step==9 || step==10 || step==11 || step==12 || step==13 || step==14 || step==15) {
				if (trials>10000000) {
					return;
				}
			}
...
int main(int argc, char const *argv[]) {
	...
	vector<int> e7Vec = convertIntToVector(19697195);
	for (int i = 0; i < 32; ++i) {
		if (e7Vec[i]==1) {
			eDifferential[7][i] = 'x';
		}
	...
	// vector<int> changeA(32, 1);
	// for (int i = 0; i < 32; ++i) {
	// 	changeA[i] = i;
	// }
	// for (int i = 0; i < HEIGHT; ++i) {
	// 	for (int j = 0; j < changeA.size(); ++j) {
	// 		if (aDifferential[i][changeA[j]]=='0') {
	// 			aDifferential[i][changeA[j]] = '1';
	// 		}
	// 		else if (aDifferential[i][changeA[j]]=='1') {
	// 			aDifferential[i][changeA[j]] = '0';
	// 		}
	// 	}
	// }
	...
	printDifferentials(true, true, true, completeDifferentials[0][2], completeDifferentials[0][1], completeDifferentials[0][0]);
	...
	for (int i = 0; i < HEIGHT-2; ++i) {
		for (int j = 0; j < 32; ++j) {
			newADifferential[i][j] = completeDifferentials[0][0][i+2][j];
			newEDifferential[i][j] = completeDifferentials[0][1][i+2][j];
			newWDifferential[i][j] = completeDifferentials[0][2][i+2][j];
		}
	}

	differentials = {newADifferential, newEDifferential, newWDifferential};
