...
void fillMessageAtStep(AVarSHA2 &aVar, EVarSHA2 &eVar, vector<unsigned int> &w, AVarSHA2 &aVar2, EVarSHA2 &eVar2, vector<unsigned int> &w2, vector<vector<vector<char> > > &differentials, Conditions &conditions, int step) {
	...
	while(!done) {
		if (checkAll) {
			...
			if (step==5) {
				if (eight!=eightStartVal) {
					lessThres=lessThres*5;
				}
				if (trials>50) {
					// cout << countNext << endl;
					return;
			...
			if (step==6) {
				if (trials>thres) {
					...
					if (eight!=eightStartVal || trials>10000) {
						checkAll = true;
					}
				}
			}
...
int main(int argc, char const *argv[]) {
	...
	unsigned int E7Fixed = 0;
	cout << "Enter E7 ";
	cin >> E7Fixed;

	vector<int> e7Vec = convertIntToVector(E7Fixed);
	for (int i = 0; i < 32; ++i) {
		if (e7Vec[i]==1) {
			eDifferential[7][i] = 'x';
		}
