...
void writeMessagesToFile(string fileName, vector<unsigned int> w, vector<unsigned int> w2) {
	ofstream myFile;
	myFile.open(fileName);
	for (int i = 0; i < w.size(); ++i) {
		myFile << w[i] << " " << w2[i] << endl;
	}
	myFile.close();
	return;
}
...
bool finalConstructionW10WithW15(vector<char>& w10Differential, vector<char> cancelWith, vector<char>& w15Differential) {
	...
	// printSingleLine(w10Differential);
	...
	while(!found && state!=-2) {
		...
		if (canCancel(w15SmallSig1, w15Vec)!=-1) {
			...
			if (finalConstructionW15(w15Vec, e7Vec) && finalConstructionW10WithW15(w10Vec, e7Vec, w15Vec)) {
				...
				w10Differential = getOppositeDifferential(e7Vec);
				found = true;
			}
		}
	}
...
void printSingleLineToFile(vector<char> v, ofstream& myFile) {
	for (int i = 0; i < v.size(); ++i) {
		myFile << v[i];
	}
	myFile << endl;
...
void printDifferentialsToFile(bool printWDiff, bool printEDiff, bool printADiff, vector<vector<char> > wDifferential, vector<vector<char> > eDifferential, vector<vector<char> > aDifferential, string fileName) {
	ofstream myFile(fileName);
	if (printWDiff) {
		myFile << "This is w differential: " << endl;
		for (int i = 0; i < wDifferential.size(); ++i) {
			if (i<10) {
				myFile << "0";
			}
			myFile << i << ": ";
			printSingleLineToFile(wDifferential[i], myFile);
			// cout << i << " & ";
	...
	if (printEDiff) {
		myFile << "This is e differential: " << endl;
		for (int i = 0; i < eDifferential.size(); ++i) {
			if (i<10) {
				myFile << "0";
			}
			myFile << i << ": ";
			printSingleLineToFile(eDifferential[i], myFile);
			// cout << i << " & ";
	...
	if (printADiff) {
		myFile << "This is a differential: " << endl;
		for (int i = 0; i < aDifferential.size(); ++i) {
			if (i<10) {
				myFile << "0";
			}
			myFile << i << ": ";
			printSingleLineToFile(aDifferential[i], myFile);
			// cout << i << " & ";
...
void setAllElse(vector<vector<char> > &aDifferential, vector<vector<char> > &eDifferential, vector<vector<char> > &wDifferential) {
	...
	// for A first
	vector<char> a7Sig = calculateFromCross(CAP_SIGMA_0, aDifferential[7]);
	vector<vector<int> > lList = getLList(a7Sig, eDifferential[8]);
	vector<int> maj8DiffNot;
	for (int i = 0; i < lList.size(); ++i) {
		for (int j = lList[i][0]; j < lList[i][1]+1; ++j) {
			if (isDifferent(aDifferential[7][j])) {
...
vector<vector<vector<vector<char> > > > getCompleteDifferentials(vector<vector<vector<char> > > partialDifferentials) {
	...
	for (int i = 0; i < possibleStates.size(); ++i) {
		...
		else if (!canFind(wDifferential[10], wDifferential[15], eDifferential[7])) {
			continue;
		}
		setAllElse(aDifferential, eDifferential, wDifferential);
		string fileName = "2.txt";
		printDifferentialsToFile(true, true, true, wDifferential, eDifferential, aDifferential, fileName);
