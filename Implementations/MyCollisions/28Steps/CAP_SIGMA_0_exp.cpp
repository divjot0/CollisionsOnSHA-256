...
bool findACancelB(vector<char>& aVec, vector<char> bVec) {
	vector<char> sigADifferential = calculateFromCross(CAP_SIGMA_0, aVec);
...
int main(int argc, char const *argv[]) {
	...
	for (int ik = 0; ik < total; ++ik) {
		...
		while(!found) {
			...
			for (int i = 0; i < 200000; ++i) {
				...
				vector<char> aAltSig0 = calculateFromCross(CAP_SIGMA_0, aAltVec);
	...
	string fileName = "CAP_SIGMA_0_exp_result.txt";
	ofstream myFile(fileName);
	myFile << "CAP_SIGMA_0 : " << totalTrials/total << endl;
	myFile.close();
	return 0;
}