...
public:
	...
	int getNumberOfConditions() {
		return conditionsStringSet.size();
	}
...
vector<vector<int> > INV_SMALL_SIGMA_0(32, vector<int> (32, -1));
vector<vector<int> > INV_SMALL_SIGMA_1(32, vector<int> (32, -1));

void getInverseSigmaFromFiles() {
	ifstream infile; 
	infile.open("inv_sigma_0.txt"); 
	for (int i = 0; i < 32; ++i) {
		for (int j = 0; j < 32; ++j) {
			infile >> INV_SMALL_SIGMA_0[i][j];
		}
	}
	infile.close();
	 
	infile.open("inv_sigma_1.txt"); 
	for (int i = 0; i < 32; ++i) {
		for (int j = 0; j < 32; ++j) {
			infile >> INV_SMALL_SIGMA_1[i][j];
		}
	}
	infile.close();
	return;
...
void setValidAssignment(vector<char>& source, Conditions conditions) {
	if (conditions.getNumberOfConditions()==0) {
		return;
	}
