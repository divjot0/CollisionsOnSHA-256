...
bool findACancelB(vector<char>& aVec, vector<char> bVec) {
	vector<char> sigADifferential = calculateFromCross(SMALL_SIGMA_1, aVec);
	...
	vector<vector<int> > sig0CausingBits = getCausingBits("SMALL_SIGMA_1");
...
int main(int argc, char const *argv[]) {
	...
	for (int ik = 0; ik < total; ++ik) {
		...
		while(!found) {
			...
			for (int i = 0; i < 200000; ++i) {
				...
				vector<char> aAltSig0 = calculateFromCross(SMALL_SIGMA_1, aAltVec);
	...
	string fileName = "SMALL_SIGMA_1_exp_result.txt";
	ofstream myFile(fileName);
	myFile << "SMALL_SIGMA_1 : " << totalTrials/total << endl;
	myFile.close();
	return 0;
}