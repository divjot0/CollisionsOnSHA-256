...
def checkSubset(A, B):
	...
	for i in range(0, 32):
		if A[i]%2==1 and B[i]%2==0:
			...
			found = False
			while(j>-1):
				if A[j]%2==0 and B[j]%2==0:
					return -1
				if A[j]%2==1 and B[j]%2==1:
					bad = bad+1
					found = True
					break
				bad = bad+1
...
while trials!=2**32:
	currentE7 = random.randrange(0, 2**32) #replace by random number
	# print(currentE7)
	trials = trials+1
	e7Vec = numToArray(currentE7)
	Sig0E7 = np.matmul(e7Vec, CAP_SIGMA_0)
	Sig1E7 = np.matmul(e7Vec, CAP_SIGMA_1)
	invsig1E7 = np.matmul(e7Vec, SMALL_SIGMA_1)
	cur = 0
	# Sig10E7 = np.matmul(Sig0E7, CAP_SIGMA_1)
	check1 = checkSubset(Sig1E7, e7Vec)
	if check1!=-1:
		count1 = count1+1
		cur = cur + check1
		check2 = checkSubset(Sig0E7, e7Vec)
		if check2!=-1:
			count2 = count2+1
			cur = cur + check2
			check3 = checkSubset(invsig1E7 , e7Vec)
			if check3!=-1:
				count3 = count3+1
				cur = cur + check3
				check4 = checkSubset(Sig1E7, invsig1E7)
				check5 = checkSubset(invsig1E7, Sig1E7)
				if check4!=-1 and check5!=-1:
					count4 = count4 + 1
					cur = cur+check4 + check5
					check6 = checkSubset(e7Vec, Sig1E7)
					if check6!=-1:
						cur = cur + check6
						count5 = count5 + 1
						if cur + hammingWeight(e7Vec) + hammingWeight(Sig0E7) + hammingWeight(Sig1E7) + hammingWeight(invsig1E7) < bestTotal:
							bestTotal = cur + hammingWeight(e7Vec) + hammingWeight(Sig0E7) + hammingWeight(Sig1E7) + hammingWeight(invsig1E7)
							argBest = currentE7
							bestBad = cur
						elif cur + hammingWeight(e7Vec) + hammingWeight(Sig0E7) + hammingWeight(Sig1E7) + hammingWeight(invsig1E7) == bestTotal and cur<=bestBad:
							argBest = currentE7
							bestBad = cur
						l.append([currentE7, cur])
	# if count5>0:
	# 	print(l)
	# 	print(bestTotal)
	# 	print(argBest),
	# 	bestVec = numToArray(argBest)
	# 	print(bestBad)
	if trials%100000==0 and argBest!=-1:
		# print(trials),
		# print(count1),
		# print(count2),
		# print(count3)
		...
		if same == 20:
			...
			same = 0
		prevArgBest = argBest
	elif trials%1000000==0:
		print(trials),
		print("None found uptil now")