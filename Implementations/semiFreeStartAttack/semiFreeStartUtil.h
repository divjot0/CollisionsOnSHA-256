...
void goBackWithFixedMessages(int step, vector<vector<vector<unsigned int> > > &transitRegisters, vector<vector<unsigned int> > &messages, unsigned int differenceBetweenMessages) {
	unsigned int K[64] = {
	...
	transitRegisters[0][0][7] = transitRegisters[0][1][0] - EP0(transitRegisters[0][0][0]) - MAJ(transitRegisters[0][0][0], transitRegisters[0][0][1], transitRegisters[0][0][2]) - EP1(transitRegisters[0][0][4]) - CH(transitRegisters[0][0][4], transitRegisters[0][0][5], transitRegisters[0][0][6]) - K[1] - messages[0][1];
	transitRegisters[0][0][3] = transitRegisters[0][1][4] - transitRegisters[0][0][7] - EP1(transitRegisters[0][0][4]) - CH(transitRegisters[0][0][4], transitRegisters[0][0][5], transitRegisters[0][0][6]) - K[1] - messages[0][1];

	transitRegisters[1][0][7] = transitRegisters[1][1][0] - EP0(transitRegisters[1][0][0]) - MAJ(transitRegisters[1][0][0], transitRegisters[1][0][1], transitRegisters[1][0][2]) - EP1(transitRegisters[1][0][4]) - CH(transitRegisters[1][0][4], transitRegisters[1][0][5], transitRegisters[1][0][6]) - K[1] - messages[1][1];
	transitRegisters[1][0][3] = transitRegisters[1][1][4] - transitRegisters[1][0][7] - EP1(transitRegisters[1][0][4]) - CH(transitRegisters[1][0][4], transitRegisters[1][0][5], transitRegisters[1][0][6]) - K[1] - messages[1][1];
}

vector<vector<unsigned int> > getIVs(vector<vector<vector<unsigned int> > > &transitRegisters, vector<vector<unsigned int> > messages) {
	vector<vector<unsigned int> > IV (2, vector<unsigned int> (8));
	for (int i = 0; i < 8; ++i) {
		if (i!=0 && i!=4) {
			IV[0][i-1] = transitRegisters[0][0][i];
			IV[1][i-1] = transitRegisters[1][0][i];
		}
	}
	unsigned int k[64] = {
		0x428a2f98,0x71374491,0xb5c0fbcf,0xe9b5dba5,0x3956c25b,0x59f111f1,
	...
	IV[0][7] = transitRegisters[0][0][0] - EP0(IV[0][0]) - MAJ(IV[0][0], IV[0][1], IV[0][2]) - EP1(IV[0][4]) - CH(IV[0][4], IV[0][5], IV[0][6]) - k[0] - messages[0][0];
	IV[1][7] = transitRegisters[1][0][0] - EP0(IV[1][0]) - MAJ(IV[1][0], IV[1][1], IV[1][2]) - EP1(IV[1][4]) - CH(IV[1][4], IV[1][5], IV[1][6]) - k[0] - messages[1][0];

	IV[0][3] = transitRegisters[0][0][4] - IV[0][7] - EP1(IV[0][4]) - CH(IV[0][4], IV[0][5], IV[0][6]) - k[0] - messages[0][0];
	IV[1][3] = transitRegisters[1][0][4] - IV[1][7] - EP1(IV[1][4]) - CH(IV[1][4], IV[1][5], IV[1][6]) - k[0] - messages[1][0];
...
string compute_hash( vector<unsigned int> block)
{
	// Taken from the spec
...
vector<unsigned int> compute_hash_step(vector<unsigned int> registers,  vector<unsigned int> block, int step)
{
	// Taken from the spec
	//   SHA-224 and SHA-256 use the same sequence of sixty-four constant
	//   32-bit words, K0, K1, ..., K63.  These words represent the first 32
	...
	// Initial Hash Values, first thirty-two bits of the fractional parts of
	// the square roots of the first eight prime numbers. 
	unsigned int  H0 = registers[0];
	unsigned int  H1 = registers[1];
	unsigned int  H2 = registers[2];
	unsigned int  H3 = registers[3];
	unsigned int  H4 = registers[4];
	unsigned int  H5 = registers[5];
	unsigned int  H6 = registers[6];
	unsigned int  H7 = registers[7];
