...
void expandMessage(vector<unsigned int> &message) {
	for(int t = 16; t <= 63; t++)
	{
		// Also taken from spec.
		message[t] = SSIG1(message[t-2]) + message[t-7] + SSIG0(message[t-15]) + message[t-16];
...
void printMessages(vector<vector<unsigned int> > &messages) {
	cout << "Message 1 : ";
	for (int i = 0; i < 23; ++i) {
		cout << show_as_hex(messages[0][i]) << " ";
	}
	cout << endl << "Message 2 : ";
	for (int i = 0; i < 23; ++i) {
		cout << show_as_hex(messages[1][i]) << " ";
	}
	cout << endl;
	return;
}
...
string compute_hash(const vector<unsigned int> block)
{
	...
	//FAKE
	// unsigned int  H0 = 0xc77405ea;
	// unsigned int  H1 = 0x8bfe2016;
	// unsigned int  H2 = 0xff0531b6;
	// unsigned int  H3 = 0xa89b81f6;
	// unsigned int  H4 = 0xe98cf052;
	// unsigned int  H5 = 0x491a6c62;
	// unsigned int  H6 = 0xfd009a40;
	// unsigned int  H7 = 0x3969dc83;
